...
| 2. 数据质量核验 | 四维质检 + 漏斗分析：通过率89.1%，驳回Top1原因AI误报(40.5%) | Pandas |
...
| 9. SQL取数验证 | 标准数仓SQL，8个查询，核心指标交叉验证 | Hive/SparkSQL/MySQL |

## 核心发现

| # | 发现 | 建议 |
|---|---|---|
| 1 | 采集21,359件 → 审核通过19,028(89.1%) → 驳回2,331(10.9%)，AI误报占驳回40.5% | 优化AI识别模型，减少误报 |
| 2 | 人行道大类占46.3%，路框差+井框差合计37.9% | 路框差/井框差专项批量治理 |
| 3 | 沙荷路密度148件/km为全区最高，被平台"总量排行"掩盖 | 排行增加「案件密度(件/km)」口径 |
| 4 | 众源车通过率仅74%，平均时滞8天，54%超7天 | T+3审核SLA + 机审预筛 |
| 5 | 周五众源跑全域，案件量+52%且质量最差 | 审核排产周五倾斜 + 快审模式 |
| 6 | 两轮车24%因模糊被驳回 | 采集防抖/补光规范 |
